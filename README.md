...
| Version | Date | Change |
|---|---|---|
| **2.9.18.1** | 17 May | **HOTFIX** — `_migrate_vehicle_origin_data_to_link` failed on bench migrate with `Fieldtype cannot be changed from Data to Link`. Frappe's Custom Field validator (`custom_field.py:191`) blocks this in `doc.save()` for safety. Fix: direct `UPDATE tabCustom Field` SQL bypassing the validator — safe because varchar(140) for Data and Link share the same on-disk column type, only metadata flips. Lesson 263 modified-bump still applied; added `frappe.clear_cache()` so the new Link picker shows up without a hard refresh. Verified: all 4 vehicle_origin CFs (TS Token / TS Gate Entry / PR / PI) now Link→TS Vehicle Origin in DB. |
| **2.9.18** | 17 May | **G1/G2 master-based capture** — client requirement to remove parallel free-text vehicle/driver/operator fields. New `TS Vehicle Origin` Master DocType (`ts_gate_entry/doctype/ts_vehicle_origin/`) with `_backfill_vehicle_origin_master()` seeder (distinct existing free-text values → Master rows) and `_migrate_vehicle_origin_data_to_link()` (flips Custom Field on TS Token / TS Gate Entry / PR / PI from Data → Link in place). New `vehicle_master` Link added to TS Token (G1) with `mandatory_depends_on` Material; `fetch_from vehicle_master.vehicle_number` / `vehicle_type` keeps legacy columns auto-populated. Driver Link unhidden on G1 with `fetch_from driver.driver_name` / `mobile_number` / `license_number`. Hidden on form (data preserved): `vehicle_number`, `driver_name` (G1 + G2), `vehicle_number_display`, `vehicle_type` (G2 echo), `driver_mobile` echo. Legacy Custom Fields hidden via `_hide_legacy_g1_custom_fields()`: `ts_driver_*_g1`, `ts_g1_operator_name`, `ts_g1_driver_*` (operator now derived from Frappe `owner`). G2 `vehicle_master` gains `fetch_from token_number.vehicle_master` so it auto-pulls from G1. No on_submit changes — GATE_ENTRY_SUBMIT lock untouched. |
